...
const ActivityPage = () => {
  ...
  useEffect(() => {
    if (completedQuizzesList.length > 0) {
      const totalPointsCalculated = completedQuizzesList.reduce(
        (acc, current) => {
          const score =
            current.score !== undefined ? Number(current.score) : 10;
  ...
  const growthRatePercentage =
    MAX_POSSIBLE_POINTS > 0
      ? Math.min(
          Math.round((totalEarnedPoints / MAX_POSSIBLE_POINTS) * 100),
          100,
        )
      : 0;
    ...
    <div className="min-h-screen p-6 lg:p-10 font-sans text-slate-800 dark:text-zinc-100 bg-transparent">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white dark:bg-[#232925] p-6 rounded-xl shadow-sm border border-gray-100 dark:border-[#2C3530]">
            <h2 className="text-[#446C4F] dark:text-[#528B63] font-bold text-xl mb-6">
              General Info
            </h2>
            <div className="space-y-4 text-gray-500 dark:text-gray-300 text-sm">
              <div className="flex justify-between items-center">
                <span>Quizzes Completed</span>
                <span className="font-bold text-[#446C4F] dark:text-[#528B63]">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-[#446C4F] dark:text-[#528B63]">
                  {loadingExercises
                    ? "..."
                    : String(completedExercisesList.length)}
                </span>
              </div>
          ...
          {/* MUI Calendar */}
          <div className="w-full bg-white dark:bg-[#232925] text-black border-gray-100 dark:text-gray-200 rounded-xl border dark:border-[#2C3530] overflow-hidden">
        ...
        {/* MAIN PANEL */}
        <div className="lg:col-span-9 bg-white dark:bg-[#232925] dark:border-[#2C3530] rounded-xl p-8 shadow-sm border border-gray-100 min-h-[600px]">
          <div className="flex flex-wrap justify-center gap-4 mb-10 border-b border-gray-100 dark:border-[#2C3530] pb-6">
            {["Weekly Progress", "Quizzes", "Exercises"].map((tab) => (
              <button
                ...
                className={`px-6 py-2 cursor-pointer rounded-xl font-semibold transition-all ${
                  activeTab === tab
                    ? "bg-[#446C4F] text-white dark:bg-[#528B63] shadow-lg translate-y-[-2px]"
                    : "bg-[#F5F7F3] text-gray-400 dark:bg-[#1A1F1C] dark:text-zinc-300 hover:bg-gray-200 dark:hover:bg-[#2C3530]"
                }`}
              >
                {tab}
              </button>
            ...
            {activeTab === "Exercises" &&
              ...
              (loadingExercises ? (
                <div className="flex justify-center items-center h-[300px]">
                  <span className="inline-block w-8 h-8 border-4 border-[#446C4F] border-t-transparent rounded-full animate-spin"></span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in zoom-in-95">
                  {exercisesList.map((ex) => {
                    const isCompleted = completedExercisesList.some(
                      (c) =>
                        Number(c.exerciseId) === Number(ex.exerciseId) ||
                        Number(c.id) === Number(ex.exerciseId),
                    );

                    return (
                      <div
                        key={ex.exerciseId}
                        className="bg-white dark:bg-[#1A1F1C] border-2 border-gray-50 dark:border-[#2C3530] rounded-xl overflow-hidden flex flex-col hover:shadow-xl transition-shadow group"
                      >
                        <div className="relative h-48 overflow-hidden bg-zinc-100 dark:bg-[#232925]">
                          <img
                            src={
                              ex.gifUrl ||
                          ...
                          />
                          {isCompleted && (
                            <span className="absolute top-3 right-3 bg-[#446C4F] text-white text-xs font-bold px-3 py-1 rounded-full shadow-md z-10">
                              ✓ Completed
                            </span>
                          )}
                        </div>
                        <div className="p-6 flex-1 flex flex-col justify-between">
                          <div>
                            <h4 className="font-bold text-xl mb-1 text-slate-800 dark:text-zinc-100">
                              {ex.title}
                            </h4>
                            ...
                            <div className="flex gap-3 mb-6 mt-3">
                              <span className="bg-[#E8F3EE] dark:bg-[#446C4F]/20 text-[#446C4F] dark:text-[#528B63] px-4 py-1 rounded-xl text-xs font-bold uppercase">
                                {ex.duration > 1
                                  ? `${ex.duration} mins`
                                  : `${ex.duration} min`}
                              </span>
                          ...
                          <button
                            ...
                            className="w-full cursor-pointer bg-[#446C4F] text-white py-3 rounded-xl font-bold hover:opacity-90 shadow-md transition-all active:scale-[0.98]"
                          >
            ...
            {selectedExercise && (
              <div className="animate-in slide-in-from-right-8 duration-300">
                <button
                  ...
                  className="flex items-center gap-2 cursor-pointer text-gray-400 dark:text-zinc-400 hover:text-[#446C4F] dark:hover:text-[#528B63] font-bold mb-8 group"
                >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                  <div className="rounded-xl overflow-hidden shadow-xl bg-zinc-100 dark:bg-[#1A1F1C] border dark:border-[#2C3530]">
                    <img
                  ...
                  <div className="space-y-8">
                    <div>
                      <h3 className="text-3xl font-black text-[#446C4F] dark:text-[#528B63] mb-2">
                    ...
                    <div className="space-y-4">
                      <h4 className="text-lg font-bold text-slate-700 dark:text-zinc-200">
                        Exercise Steps:
                      </h4>
                        ...
                        selectedExercise.steps
                          ...
                          .map((step, index) => {
                            ...
                            return (
                              <div
                                key={index}
                                className="bg-[#F5F7F3] dark:bg-[#1A1F1C] p-4 rounded-xl border border-gray-100 dark:border-[#2C3530] shadow-sm flex gap-4 items-start"
                              >
                                <div className="bg-white dark:bg-[#232925] min-w-[32px] h-8 flex items-center justify-center rounded-full font-bold text-[#446C4F] dark:text-[#528B63] shadow-sm border dark:border-[#2C3530]">
                                  {index + 1}
                                </div>
                    ...
                    <div className="pt-4">
                      <button
                        ...
                        onClick={() =>
                          handleCompleteExercise(selectedExercise.exerciseId)
                        }
                        className={`w-full cursor-pointer bg-[#446C4F] text-white py-4 rounded-xl font-bold shadow-lg transition-all ${
                          submittingExercise
