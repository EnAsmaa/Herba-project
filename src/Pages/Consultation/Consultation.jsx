...
export default function Consultation() {
 ...
 return (
    <>
      <section className="min-h-screen bg-gray-50 text-[#3E4E36] dark:bg-[#1A1F1C] dark:text-[#E2E8F0] py-9 font-sans transition-colors duration-200">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div className="consult-head">
              ...
              <p className="text-xs md:text-sm text-[#3E4E36]/70 dark:text-[#94A3B8]">
                Book your session with the best certified consultants and specialists
              </p>
            </div>
        ...
        <div className="container mx-auto px-4 lg:px-7 py-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {doctors?.map(doc => (
              <Link key={doc.id} to={`/ask-consultation/${doc.id}`} className="group">
                <div className="consult-content flex flex-col items-center text-center shadow-sm bg-white dark:bg-[#232925] border border-gray-100 dark:border-[#2C3530] rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-md relative">
                  ...
                  {/* Avatar */}
                  <div className="w-16 h-16 rounded-2xl bg-green-50 dark:bg-green-950/20 flex items-center justify-center text-[#446C4F] dark:text-[#528B63] font-black text-xl uppercase border border-green-100 dark:border-green-900/10 shadow-inner group-hover:scale-105 transition-transform duration-300">
                  ...
                  {/* Rate Count Info */}
                  <p className="text-gray-400 dark:text-zinc-500 text-xs mt-1.5 font-medium">
                    ({doc.rateCount} reviews) • <span className="font-bold text-[#3E4E36] dark:text-[#E2E8F0]">{doc.rating}</span>
                  </p>
